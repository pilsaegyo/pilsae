...
const worker = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/track-visit" && request.method === "POST") {
        return recordVisitorSession(request, env, origin);
      }

      if (url.pathname === "/visitor-stats" && request.method === "GET") {
        requireAdmin(request, env);
        const stats = await visitorAnalyticsStats(env);
        return jsonResponse({ ok:true, ...stats }, 200, env, origin);
      }
  ...
  async scheduled(controller, env, ctx) {
    ...
    if (cron === "0 18 * * 6") {
      ...
      await cleanupVisitorAnalytics(env);
      return;
    }
...
let visitorSchemaInitPromise = null;

function visitorKstDay(value=Date.now()) {
...
async function visitorIdFromRequest(request, env) {
  const ip = String(request.headers.get("CF-Connecting-IP") || "").trim();
  if (!ip) return "";
  const salt = String(env.ADMIN_TOKEN || "pilsae-visitor");
...
function visitorLooksLikeBot(request) {
  const ua = String(request.headers.get("User-Agent") || "");
  return /bot|crawler|spider|slurp|preview|facebookexternalhit|bingpreview|headless/i.test(ua);
}

async function ensureVisitorSchema(env) {
  if (!env.VISITOR_DB) return false;
  if (!visitorSchemaInitPromise) {
    visitorSchemaInitPromise = env.VISITOR_DB.batch([
      env.VISITOR_DB.prepare(`CREATE TABLE IF NOT EXISTS visitors (
        visitor_id TEXT PRIMARY KEY,
        first_seen_at TEXT NOT NULL,
        last_seen_at TEXT NOT NULL,
        visit_count INTEGER NOT NULL DEFAULT 1
      )`),
      env.VISITOR_DB.prepare(`CREATE TABLE IF NOT EXISTS visits (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        visitor_id TEXT NOT NULL,
        visited_at TEXT NOT NULL,
        day_kst TEXT NOT NULL
      )`),
      env.VISITOR_DB.prepare(`CREATE INDEX IF NOT EXISTS idx_visits_day_kst ON visits(day_kst)`),
      env.VISITOR_DB.prepare(`CREATE INDEX IF NOT EXISTS idx_visits_visited_at ON visits(visited_at DESC)`),
      env.VISITOR_DB.prepare(`CREATE INDEX IF NOT EXISTS idx_visits_visitor_id ON visits(visitor_id)`),
      env.VISITOR_DB.prepare(`CREATE TABLE IF NOT EXISTS visitor_totals (
      ...
      await env.VISITOR_DB.prepare(`
        INSERT OR IGNORE INTO visitor_totals(id, total_visits, updated_at)
        SELECT 1, COUNT(*), ? FROM visits
      `).bind(nowIso).run();
      return true;
    }).catch(error => {
      visitorSchemaInitPromise = null;
      throw error;
    });
  }
  return visitorSchemaInitPromise;
}

async function recordVisitorSession(request, env, origin) {
  if (!env.VISITOR_DB) return new Response(null, { status:204, headers:corsHeaders(env, origin) });
  const allowed = env.ALLOWED_ORIGIN || "https://pilsae.hyesung.workers.dev";
  if (origin && origin !== allowed) {
    return new Response(null, { status:204, headers:corsHeaders(env, origin) });
  }
  if (visitorLooksLikeBot(request)) {
    return new Response(null, { status:204, headers:corsHeaders(env, origin) });
  }

  await ensureVisitorSchema(env);
  const visitorId = await visitorIdFromRequest(request, env);
  if (!visitorId) return new Response(null, { status:204, headers:corsHeaders(env, origin) });

  const now = new Date();
  const nowIso = now.toISOString();
  const prior = await env.VISITOR_DB.prepare(
    `SELECT last_seen_at FROM visitors WHERE visitor_id = ? LIMIT 1`
  ).bind(visitorId).first();

  // Treat repeated loads from the same IP within 30 minutes as one visit session.
  if (prior?.last_seen_at) {
    const last = new Date(prior.last_seen_at).getTime();
    if (Number.isFinite(last) && now.getTime() - last < 30 * 60 * 1000) {
      return new Response(null, { status:204, headers:corsHeaders(env, origin) });
    }
  }

  const dayKst = visitorKstDay(now);
  await env.VISITOR_DB.batch([
    env.VISITOR_DB.prepare(`
      INSERT INTO visitors(visitor_id, first_seen_at, last_seen_at, visit_count)
      VALUES (?, ?, ?, 1)
      ON CONFLICT(visitor_id) DO UPDATE SET
        last_seen_at = excluded.last_seen_at,
        visit_count = visitors.visit_count + 1
    `).bind(visitorId, nowIso, nowIso),
    env.VISITOR_DB.prepare(`
      INSERT INTO visits(visitor_id, visited_at, day_kst)
      VALUES (?, ?, ?)
    `).bind(visitorId, nowIso, dayKst),
    env.VISITOR_DB.prepare(`
...
async function visitorAnalyticsStats(env) {
  if (!env.VISITOR_DB) {
    return { configured:false, retentionDays:90 };
  }
  await ensureVisitorSchema(env);

  const now = Date.now();
  const today = visitorKstDay(now);
  const cutoff7 = new Date(now - 7 * 24 * 60 * 60 * 1000).toISOString();
  const cutoff90 = new Date(now - 90 * 24 * 60 * 60 * 1000).toISOString();
  const dailyStart = visitorKstDay(now - 13 * 24 * 60 * 60 * 1000);

  const [lifetimeRow, todayRow, weekRow, unique90Row, dailyRows, recentRows, topRows] = await Promise.all([
    env.VISITOR_DB.prepare(`
      SELECT total_visits FROM visitor_totals WHERE id = 1
    `).first(),
    env.VISITOR_DB.prepare(`
      SELECT COUNT(*) AS visits, COUNT(DISTINCT visitor_id) AS unique_visitors
      FROM visits WHERE day_kst = ?
    `).bind(today).first(),
    env.VISITOR_DB.prepare(`SELECT COUNT(*) AS visits FROM visits WHERE visited_at >= ?`).bind(cutoff7).first(),
    env.VISITOR_DB.prepare(`SELECT COUNT(DISTINCT visitor_id) AS unique_visitors FROM visits WHERE visited_at >= ?`).bind(cutoff90).first(),
    env.VISITOR_DB.prepare(`
      SELECT day_kst AS day, COUNT(*) AS visits, COUNT(DISTINCT visitor_id) AS unique_visitors
      FROM visits WHERE day_kst >= ?
      GROUP BY day_kst ORDER BY day_kst ASC
    `).bind(dailyStart).all(),
    env.VISITOR_DB.prepare(`
      SELECT visits.visitor_id, visits.visited_at, visitors.visit_count
      FROM visits LEFT JOIN visitors ON visitors.visitor_id = visits.visitor_id
      ORDER BY visits.visited_at DESC LIMIT 50
    `).all(),
    env.VISITOR_DB.prepare(`
      SELECT visitor_id, COUNT(*) AS visits, MAX(visited_at) AS last_seen_at
      FROM visits WHERE visited_at >= ?
      GROUP BY visitor_id ORDER BY visits DESC, last_seen_at DESC LIMIT 50
    `).bind(cutoff90).all()
  ]);
  ...
  return {
    ...
    generatedAt:new Date().toISOString(),
    lifetime:{
      visits:Number(lifetimeRow?.total_visits || 0)
    },
    today:{
      visits:Number(todayRow?.visits || 0),
      unique:Number(todayRow?.unique_visitors || 0)
    },
    last7Days:{ visits:Number(weekRow?.visits || 0) },
    last90Days:{ unique:Number(unique90Row?.unique_visitors || 0) },
...
async function cleanupVisitorAnalytics(env) {
  if (!env.VISITOR_DB) return;
  await ensureVisitorSchema(env);
  const cutoff = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString();
  await env.VISITOR_DB.batch([
    env.VISITOR_DB.prepare(`DELETE FROM visits WHERE visited_at < ?`).bind(cutoff),
    env.VISITOR_DB.prepare(`DELETE FROM visitors WHERE last_seen_at < ?`).bind(cutoff)
  ]);
  // The lifetime counter is intentionally not deleted when 90-day detail
